...
Ask ten people what an "AI agent" is and you'll get ten answers: a chatbot, a clever prompt, a workflow, a model with tools. The most useful definition turns out to be the simplest one: **An agent is a large language model (LLM) running inside a harness.** 

The model is the part that you're probably already familiar with: GPT 5.5, Opus 4.8, etc. So let's spend some time on the other half: the harness. The harness is what the LLM uses to interact with the rest of the world and drive itself towards completing the tasks you give it. This is what makes a model useful for real work, and it's essential to building autonomous agents and workflows.

Flue ships a programmable TypeScript agent harness out of the box. It was the first framework built entirely around this architecture, and we think it's where agents are headed.
...
- **We want the agent to keep its work** — so we give it a *filesystem*: a workspace to read and write, somewhere to offload material too big for its context, and a place to leave notes it can return to later.
- **We want the agent to act, not just describe** — so we give it *tools*, and especially a general-purpose one like a shell. Rather than pre-building a tool for every possible action, you hand the model a computer and let it figure out the rest.
- **We want the agent to act safely** — so those actions run in a *sandbox*: an isolated, disposable environment with sensible defaults already installed, where a mistake can't reach anything it shouldn't.
- **We want the agent to know things it wasn't trained on** — so the harness gives it *memory* and *retrieval*: files it reads on startup and updates as it learns, plus ways to look up current information.
- **We want the agent to stay sharp across long work** — so the harness does *context engineering*: summarizing history before the window fills, and revealing detailed instructions only when a task actually calls for them.
- **We want the agent to do more than one thing at a time** — so it can hand focused work off to *subagents* and pull the results back together.

Flue's built-in agent harness gives you all of these pieces in TypeScript, ready to run on the deployment target of your choice:
...
Building an agent is really about building up the context that you provide your agent — choosing the model, instructions, tools, skills, and environment it runs in.
...
- [Harness](/docs/guide/harness/) — initializing an agent's environment and opening sessions.
